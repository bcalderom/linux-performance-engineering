...
- **Context switch**  
  The act of saving the CPU state of the task that has already been stopped (for any reason) and restoring the CPU state of the next task so it can run on the CPU.
...
- **Preemption**  
  Forcibly interrupting a running task so another can run, often due to priority or timeslice expiration. It always leads to a context switch.
