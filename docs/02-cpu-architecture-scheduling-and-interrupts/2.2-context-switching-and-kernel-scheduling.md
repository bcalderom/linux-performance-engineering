...
- **Interrupt Context** → See **section 2.3.2.1**
  A special CPU execution mode where no process is "current" and code cannot sleep. Interrupts preempt all tasks.

- **SoftIRQ / Hard IRQ** → See **section 2.3**
  Interrupt processing mechanisms. Hard IRQs run with interrupts disabled; SoftIRQs handle deferred work. Both preempt scheduler classes.

- **Priority Hierarchy** → See **section 2.3.6**
  The system-wide execution order: Hard IRQ → SoftIRQ → ksoftirqd → DEADLINE → RT → CFS → BATCH → IDLE.
...
> **Cross-Reference:**
> For the detailed mechanics of interrupt processing (ISR, softirq flow, interrupt context, IDT, NAPI, MSI-X) and the **full system-wide priority hierarchy** (Hard IRQ → SoftIRQ → ksoftirqd → scheduler classes), see **2.3: Interrupt Handling & IRQ Mechanisms**, particularly **section 2.3.6**.
...
> ⚠️ **Priority Note: Scheduler Classes Are Below Interrupt Priority**
>
> Linux task priority (nice values, CFS fairness, or real-time scheduling) only affects **which task runs next**.
> It does **not** affect interrupt handling.
>
> - Hard IRQs and SoftIRQs **always preempt tasks**, including RT and DEADLINE tasks.
> - A `nice -20` CFS task cannot outrun network interrupt processing.
> - A `SCHED_FIFO` priority 99 task will still be interrupted by every hardware IRQ.
>
> **Full priority hierarchy:** `Hard IRQ → SoftIRQ → ksoftirqd → DEADLINE → RT → CFS → BATCH → IDLE`
>
> See **section 2.3.6** for the complete explanation of how priority works across interrupts and the scheduler.
