...
- **RCU (Read-Copy-Update) stall**  
  A synchronization mechanism used by the kernel allowing lock-free reads. An **RCU stall** occurs when a CPU holds a reference for too long, preventing memory reclamation and freezing kernel subsystems.
