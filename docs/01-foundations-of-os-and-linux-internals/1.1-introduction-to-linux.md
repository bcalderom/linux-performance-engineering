...
## 1.1.2 Monolithic vs Microkernel vs Hybrid
...
## 1.1.3 Kernel Space vs User Space
...
> The helper script `scripts/section01_01_inspect_linux_basics.sh` automates several of these checks.

---

## 1.1.5 Observing the Running Kernel
...
## 1.1.6 Typical Failure Scenarios at This Level
...
```bash
bash scripts/section01_01_inspect_linux_basics.sh
```
...
These foundations are critical for the next topics:

- **Processes and scheduling** (1.2): How Linux decides which threads run on which CPUs.
- **Filesystem layout and kernel-relevant paths** (1.3): Where the kernel exposes state and where logs end up.
