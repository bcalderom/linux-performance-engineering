...
You will learn the detailed mechanics later; for now, remember at a high level:

- The scheduler maintains **per-CPU runqueues** of runnable tasks.
- It periodically picks a task to run, trying to give each task a fair share of CPU time (tracked internally using a concept called **virtual runtime**).
