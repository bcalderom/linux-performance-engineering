...
## 1.2.2 Linux Scheduling (High-Level)
...
## 1.2.3 Systemd & Service Management
...
## 1.2.4 Failure Scenarios and How They Manifest
...
## 1.2.5 Hands-On Exercises

These exercises use both manual commands and the helper script `scripts/section01_02_process_tree_and_scheduling.sh`.
...
```bash
bash scripts/section01_02_process_tree_and_scheduling.sh
```
...
- Understand at a high level how the **scheduler** shares CPU time.
- Use `systemd` tools to inspect **service health** and logs.
- Run small experiments that create CPU load and interpret their impact on load average and responsiveness.

These skills are foundational for analyzing CPU saturation, latency spikes, and service outages in real systems.
