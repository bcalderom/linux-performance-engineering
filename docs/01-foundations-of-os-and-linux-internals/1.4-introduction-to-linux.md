...
- **Linux kernel**: The core, privileged part of the OS that manages hardware and provides services (processes, memory, filesystems, networking) to user-space programs.
- **User space**: Where regular programs (shells, daemons, applications) run with restricted privileges, relying on the kernel for hardware access.
- **Kernel space**: The privileged execution mode where the kernel runs and can directly control hardware.
- **System call (syscall)**: A controlled request from user space to the kernel to perform a privileged operation (e.g., open a file, send a packet, allocate memory).
- **Monolithic kernel**: A kernel design where most OS services (scheduling, memory, filesystems, networking, drivers) run together in one privileged address space.
- **Syscall tracing (`strace`)**: A tool that records which syscalls a process makes and how much time it spends in them.
...
Performance engineering on Linux largely boils down to **observing and influencing the behavior** of these kernel subsystems as they serve user-space workloads. Here, "ring 0" and "ring 3" simply refer to different CPU privilege levels: kernel space (fully trusted) versus user space (restricted), which you will gradually learn more about.
...
Linux is a **monolithic kernel** with support for **loadable kernel modules** (LKMs). In performance terms, this means (at a high level):
...
- You gain **high performance and flexibility**, at the cost of needing good observability into basic kernel behavior (which this curriculum will build up gradually).
...
2. **Context switch cost**:
   ...
   - High syscall rates or frequent blocking operations can significantly impact latency and CPU usage. You do not need the micro-architectural details yet; for now, remember that **more crossings usually mean more overhead**.
...
- Some CPU work to change modes and check permissions.
- Potential contention on locks, queues, or hardware resources.
...
- **Softlockups / Hardlockups**: Situations where CPU cores appear stuck in kernel code for too long. Watchdog mechanisms emit warnings, load average spikes, and useful work can stall. For now, treat these as severe kernel-side stalls that require log and metric inspection rather than application debugging.
...
- Kernel state: `/proc` and `/sys`, which expose live kernel and process information as virtual files.
