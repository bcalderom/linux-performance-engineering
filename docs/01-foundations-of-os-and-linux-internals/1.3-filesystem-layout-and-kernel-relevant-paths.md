...
The helper script `scripts/section01_03_proc_sys_explorer.sh` automates several examples in this document.

---

## 1.3.1 High-Level Filesystem Layout
...
## 1.3.2 /proc – Kernel and Process Information
...
## 1.3.3 /sys – Kernel Objects and Tunables
...
## 1.3.6 Failure Scenarios and Diagnostics
...
## 1.3.7 Hands-On Exercises

These exercises use both manual exploration and the helper script `scripts/section01_03_proc_sys_explorer.sh`.
...
```bash
bash scripts/section01_03_proc_sys_explorer.sh
```
...
- Which directories are most relevant for **kernel state** and **diagnostics**.
- How to use `/proc` to obtain detailed information about **memory**, **CPUs**, and **processes**.
- How `/sys` exposes kernel objects and tunables and why it must be handled carefully.
- Where to look in `/var/log` and `/run` when diagnosing failures and startup issues.

These skills, combined with the process and scheduling knowledge from 1.2, give you a practical foundation for deeper dives into CPU, memory, networking, and storage performance.
